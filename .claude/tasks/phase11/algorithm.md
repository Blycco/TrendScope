...
- [x] `pipeline.py` `_stage_score`에서 그룹 제목 생성 로직 변경
- [x] 현재: 대표 기사의 `title`을 그대로 `news_group.title`로 저장 (`pipeline.py:425`)
- [x] 변경: 클러스터 내 기사들의 키워드 중 빈도·중요도 최상위 키워드를 그룹 제목으로 사용
- [x] 예: "더 오래 살지만 더 오래 아프다… 유병장수 시대 넘으려면" → "유병장수"
- [x] 기사 원문 제목은 `news_article.title`에 이미 보존되어 있으므로 데이터 손실 없음

**완료**: Ref #132. Counter 기반 빈도순 상위 3개 키워드 조합으로 그룹 제목 생성
**영향 파일**: `backend/processor/pipeline.py`
