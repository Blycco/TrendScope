...
async def get_current(
    ...
    try:
        ...
        logger.error("get_current_subscription_failed", error=str(exc))
        raise http_error(
            ErrorCode.DB_ERROR,
...
async def checkout(
    ...
    try:
        ...
        raise http_error(ErrorCode.DB_ERROR, "Checkout failed", status_code=500) from exc
...
async def cancel(
    ...
    try:
        ...
        logger.error("cancel_subscription_failed", error=str(exc))
        raise http_error(ErrorCode.DB_ERROR, "Cancellation failed", status_code=500) from exc
