...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="dialog" aria-modal="true" aria-describedby="plangate-dialog-desc" bind:this={dialogEl}>
		<div class="mx-4 w-full max-w-md rounded-lg bg-white dark:bg-gray-800 p-6 shadow-xl">
			<div class="flex items-start justify-between">
				<h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{$t('modal.plan_required.title')}</h2>
				<button onclick={onClose} class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label={$t('a11y.close_dialog')}>
					<X size={20} />
				</button>
			</div>

			<p id="plangate-dialog-desc" class="mt-3 text-sm text-gray-600 dark:text-gray-400">
